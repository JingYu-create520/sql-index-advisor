...
- It prefers silence to guessing. Known gaps, listed rather than papered over:
  ...
  - Correlated subqueries are not expanded *when rewriting a statement*: SIA006 degrades to a template rather than risk a rewrite that changes the result set. Their tables are analysed (see below).
  - A filter on a derived table's alias — `WHERE d.total > 10` over `FROM (SELECT ...) d` — names no physical table, so it is reported instead of guessed at. The body behind the alias is analysed on its own.
  - `UNION` without parentheses is still only the first branch, because a trailing `ORDER BY`/`LIMIT` belongs to the union result and not to the last query. A bracketed branch is a statement of its own and is analysed.
...
**The table behind a subquery was never examined.** `WHERE permission_id IN (SELECT role_id FROM upms_user_role WHERE user_id = ?)` is two statements needing indexes on two tables, and only one of them was analysed: the inner table produced no advice, just a note explaining the silence. Same for `FROM (SELECT ...) d`. Four audited open-source MyBatis projects — 378 mapper XML files, 1,666 statement records — hide 17 such bodies between them. Uncommon, and each one is a permissions lookup or a report page where the missing index is real. Two that came out of that audit: `paicoding`'s `EXISTS (SELECT 1 FROM column_article ca WHERE ca.article_id = a.id AND ca.column_id = ?)` now gets `(article_id, column_id)`, and `zheng`'s `upms_user_role`, whose DDL has nothing but a primary key, gets `ADD INDEX (user_id)` for the inner scan of its role lookup. Bodies are parsed as statements of their own at any nesting depth, and one thing is deliberately dropped to make that safe: inside a subquery, a bare column on the right of `=` is not treated as an index candidate, because MySQL resolves it inner-first and it may be the outer query's column — DDL for a column the table does not have is worse than no DDL. Still open, and said out loud in the report: a filter on a derived table's alias names no physical table, and an unbracketed `UNION` branch is still not analysed.

**One functional index made the schema file unreadable.** MySQL reports `COLUMN_NAME = NULL` for an expression key part — a functional index, a multi-valued JSON index, or the second part of `(user_id, UPPER(status))` — so `examples/schema-dump.sql` legitimately writes `"columns": [null]`, and the loader validated index parts as `string[]`. Validation is all-or-nothing, so one such index anywhere in the database meant `schema 校验失败` and no schema-gated rules at all. The same class of mistake had already been fixed once in this file for `length` and `charset` nulls; what makes it recurring is the fixture, not the schema — a hand-written `schema.json` never contains a shape its author had not thought of, so `tests/fixtures/schema-functional.json` is now a real 8.0.46 dump instead. `null` is a valid key part and means "this part has no column name": it cannot serve a plain-column lookup, SIA003 and SIA007 refuse to reason across it, and where SIA001 names such an index it prints `〈表达式〉` / `(expression)`, never the word `null`.

**…and it recommended that index a second time.** Create the functional index SIA004 suggests, re-export the schema, run it again: the same `ALTER TABLE orders ADD INDEX idx_orders_create_time …` came back, which in a migration file is an `ERROR 1061` on duplicate key name. An expression part's text sits in an `EXPRESSION` column that a 5.7 server does not have, so the shared dump cannot carry it and the only handle left is the name — the rule now suppresses the DDL when the table already carries an index with the name this advice would use, drops to `warn`, and says out loud that a name match is not proof, with `SHOW INDEX` as the check.

**A deep-pagination rewrite that did not run.** For `SELECT id, user_id, amount FROM orders WHERE … LIMIT 100000, 20` — no table alias in the original — the deferred join SIA006 handed back was `… AS page JOIN orders ON `id` = page.`id``, and MySQL answered `ERROR 1052 (23000): Column 'id' in on clause is ambiguous`, because the derived table exposes `id` as well. It had also replaced a three-column projection with `SELECT *`, so even the version that parsed would have returned columns nothing asked for. This one was caught by doing the obvious thing the test suite had never done: execute the emitted rewrite against a live 8.0.46. The rewrite now carries its own alias (`t`) and qualifies the join, the projection and the outer sort through it, and it is **withdrawn** — no `rewrite` field, template plus reason in the message — whenever the projection cannot be carried over honestly: a function, an `AS` rename, `DISTINCT`, or a sort key that cannot be re-qualified. `SELECT amount + 0` was the case that turned this into a parse-level flag: the expression contains a column, and treating the projection as that column would have changed a returned value.

**Two rewrites that changed the answer.** Both turned up the same way — asking a real MySQL whether the two forms select the same rows, which the suite had never done. `LEFT(code, 3) = 123` became `code LIKE '2%'`: the code cut a leading and trailing character off something that had no quotes to strip, so the pattern lost its first digit, and a numeric comparison in MySQL is not a prefix test at all. `LEFT(code, 3) = remark` became `code LIKE 'emar%'`. Both run without error and select different rows, which is the worst kind of advice this tool can give. A rewrite now requires a genuine string literal, and gives nothing for a number, a column reference or an unreadable value. The same probe found that `DATE(create_time) = '2026-09-17 13:00:00'` matches no row at all on 8.0.46 (`DATE()` is midnight; the comparison is against the whole literal — the day window returns 3 rows, that predicate returns 0), so instead of a rewrite the tool says the predicate is dead and writes the day window it probably meant.
...
node scripts/verify-rewrites.mjs   # needs a MySQL container; see below
```

299 tests. Beyond hand-written cases, `tests/fuzz.test.ts` generates about 1,200 statements — a third of them carrying a correlated `IN (SELECT ...)` or `EXISTS (SELECT ...)` of their own — plus a list of deliberately malformed ones, and asserts the properties that must hold for any input: never throw, never index a column that does not exist, never advise a table no statement named, never propose an index another already covers, and produce byte-identical output on repeated runs. That suite is what caught the tokenizer reading `1e999` as `1` plus a column named `e999`, and signed literals splitting one query pattern into two fingerprints. Fixtures under `tests/fixtures/` are real-shaped MySQL 8.0 logs, including a messy one with administrator commands, multi-line statements and an unterminated tail, and a `schema.json` dumped from a live 8.0.46 that carries a functional index, a multi-valued JSON index and a FULLTEXT index — the shapes a hand-written fixture never thinks to include.

`scripts/verify-rewrites.mjs` covers the one thing a test cannot: whether the SQL the tool suggests **executes, and selects the same rows**. It starts nothing itself — run it against the demo container from the quickstart (`docker run -d --name sia-mysql -e MYSQL_ROOT_PASSWORD=sia -e MYSQL_DATABASE=demo mysql:8.0`) — and it exits non-zero if any rewrite differs, comparing primary-key sets rather than row counts. Both of the rewrite defects below were found by it.
