...
var SEVERITY_ORDER, DEFAULT_RULE_OPTIONS;
...
var WORD_START, WORD_BODY, PUNCT_TWO, PUNCT_ONE, SQL_KEYWORDS;
var init_token = __esm({
  "src/parsers/token.ts"() {
    "use strict";
    WORD_START = /[A-Za-z_$一-龥]/;
    WORD_BODY = /[A-Za-z0-9_$一-龥]/;
    PUNCT_TWO = /* @__PURE__ */ new Set(["!=", "<>", ">=", "<=", "&&", "||"]);
    PUNCT_ONE = "(),;=<>!+-*/%&|^~.:[]{}?\\";
    SQL_KEYWORDS = /* @__PURE__ */ new Set(["in", "not", "and", "or", "is", "like", "between", "using", "values"]);
  }
});
...
var RESERVED, AGGREGATES, STATEMENT_STARTERS, SET_OPERATORS, JOIN_PARTICLES, RANGE_PUNCT;
var init_sql = __esm({
  "src/parsers/sql.ts"() {
    "use strict";
    init_fingerprint();
    ...
    AGGREGATES = /* @__PURE__ */ new Set(["count", "sum", "avg", "min", "max", "group_concat"]);
    STATEMENT_STARTERS = /* @__PURE__ */ new Set(["select", "insert", "update", "delete", "replace", "with"]);
    SET_OPERATORS = /* @__PURE__ */ new Set(["union", "intersect", "except"]);
    JOIN_PARTICLES = /* @__PURE__ */ new Set([
      "left",
      "right",
      "inner",
      "cross",
...
var RE_TIME, RE_USER_HOST, RE_QUERY_TIME, RE_LOCK_TIME, RE_ROWS_SENT, RE_ROWS_EXAMINED, RE_SCHEMA;
var init_slowlog = __esm({
  "src/parsers/slowlog.ts"() {
    "use strict";
    init_fingerprint();
    init_sql();
    RE_TIME = /^#\s*Time:\s*(.+)$/;
    RE_USER_HOST = /^#\s*User@Host:\s*(.+)$/;
    RE_QUERY_TIME = /#\s*Query_time:\s*([0-9.]+)/;
    RE_LOCK_TIME = /Lock_time:\s*([0-9.]+)/;
    RE_ROWS_SENT = /Rows_sent:\s*([0-9]+)/;
    RE_ROWS_EXAMINED = /Rows_examined:\s*([0-9]+)/;
    RE_SCHEMA = /^#\s*Schema:\s*(.*)$/;
  }
});
...
var MAX_VARIANTS, STATEMENT_TAGS, ENTITIES;
...
var optionalText, optionalNumber, columnSchema, indexSchema, tableSchema, schemaFileSchema;
var init_loader = __esm({
  "src/schema/loader.ts"() {
    "use strict";
    optionalText = z.string().nullish();
    optionalNumber = z.number().nullish();
...
var EQ_OPS, PREDICATE_SCOPES, RANGE_OPS, PRACTICAL_MAX_PREFIX_CHARS;
var init_helpers = __esm({
  "src/rules/helpers.ts"() {
    ...
    init_token();
    init_loader();
    EQ_OPS = /* @__PURE__ */ new Set(["=", "in", "in-subquery", "like-prefix"]);
    PREDICATE_SCOPES = /* @__PURE__ */ new Set([
      "where-eq",
      "where-in",
      "where-range",
      "where-like-prefix",
    ...
    RANGE_OPS = /* @__PURE__ */ new Set([">", "<", ">=", "<=", "!=", "<>", "between", "like-middle"]);
    PRACTICAL_MAX_PREFIX_CHARS = 128;
  }
});
...
var RULE_ID, sia001, FLAG_NAMES, FLAG_CAUTION;
var init_sia001 = __esm({
  "src/rules/sia001.ts"() {
    "use strict";
    init_helpers();
    init_loader();
    ...
    sia001 = {
      ...
      run(ctx) {
        ...
        for (const bucket of buckets) {
          ...
          const severity2 = lowCardinalityRisk ? "info" : table ? prefixHit ? "warn" : "error" : "info";
          findings.push({
            rule: RULE_ID,
            severity: severity2,
            sql: truncateSql(record.parsed.sql),
    ...
    FLAG_NAMES = /^(is_|has_|can_|enabled?|disabled|deleted?|synced?|verified|activated?|expired?|valid|invalid|active|inactive|state|status|type|kind|flag)$/;
    FLAG_CAUTION = "\u6CE8\u610F\uFF1A\u8FD9\u91CC\u7684\u5217\u5168\u662F\u5E03\u5C14/\u6807\u5FD7\u4F4D\u7C7B\u578B\uFF0C\u7EC4\u5408\u8D77\u6765\u7684\u533A\u5206\u5EA6\u4E5F\u53EF\u80FD\u6781\u4F4E\uFF0C\u4F18\u5316\u5668\u672A\u5FC5\u4F1A\u9009\u5B83\u3002\u5148\u8DD1 SELECT COUNT(DISTINCT \u52171, \u52172)/COUNT(*) FROM \u8868; \u786E\u8BA4\u6BD4\u503C\u8DB3\u591F\u5C0F\u518D\u5EFA\u3002";
  }
});

// src/rules/sia002.ts
var RULE_ID2, sia002;
var init_sia002 = __esm({
  "src/rules/sia002.ts"() {
    "use strict";
    init_helpers();
    init_loader();
...
// src/rules/sia007.ts
var RULE_ID7, ROW_PRESSURE, sia007;
var init_sia007 = __esm({
  "src/rules/sia007.ts"() {
    "use strict";
    init_helpers();
    init_loader();
...
var init_registry = __esm({
  "src/rules/registry.ts"() {
    ...
    init_sia006();
    init_sia007();
    ALL_RULES = [sia001, sia002, sia003, sia004, sia005, sia006, sia007];
  }
});
...
function sortFindings(findings, records) {
  ...
  return [...findings].sort((a, b) => {
    const severity2 = SEVERITY_ORDER[b.severity] - SEVERITY_ORDER[a.severity];
    if (severity2 !== 0) return severity2;
...
function rank2(severity2) {
  return severity2 === "error" ? 3 : severity2 === "warn" ? 2 : 1;
}
...
var INSTRUCTIONS, schemaInput, severitySchema, emitSqlSchema, NOTHING_ANALYSABLE;
var init_tools = __esm({
  "src/mcp/tools.ts"() {
    ...
    schemaInput = z2.string().optional().describe("schema.json \u7684\u5185\u5BB9\uFF08JSON \u5B57\u7B26\u4E32\uFF09\uFF0C\u6216\u672C\u673A\u4E0A\u7684\u8DEF\u5F84");
    severitySchema = z2.enum(["error", "warn", "info"]).optional();
    emitSqlSchema = z2.boolean().optional().describe("\u4E3A true \u65F6\u9644\u5E26\u53BB\u91CD\u540E\u7684\u8FC1\u79FB SQL");
    NOTHING_ANALYSABLE = "\u6CA1\u6709\u89E3\u6790\u51FA\u4EFB\u4F55\u53EF\u5206\u6790\u7684\u8BED\u53E5\uFF08\u652F\u6301 SELECT / INSERT / UPDATE / DELETE\uFF09\u3002";
  }
});
...
__export(mcp_exports, {
  ...
  VERSION: () => VERSION,
  analyzeMapperTool: () => analyzeMapperTool,
  analyzeSlowLogTool: () => analyzeSlowLogTool,
  analyzeSqlTool: () => analyzeSqlTool,
  createServer: () => createServer,
  emitSqlSchema: () => emitSqlSchema,
  explainRulesTool: () => explainRulesTool,
  loadSchemaInput: () => loadSchemaInput,
  main: () => main,
...
var GENERIC = "\u5148\u628A\u8FD9\u6761\u5EFA\u8BAE\u5F53\u6210\u5047\u8BBE\uFF1A\u7528 EXPLAIN \u9A8C\u8BC1\u4E00\u6B21\uFF0C\u518D\u51B3\u5B9A\u8981\u4E0D\u8981\u843D\u5230\u7EBF\u4E0A\u3002";
var mockProvider = {
  name: "mock",
  async complete(prompt) {
    const rule = /^规则：(SIA\d{3})/m.exec(prompt)?.[1];
    const title = rule ? ruleById(rule)?.title : void 0;
...
async function polish(result, provider = mockProvider, limit = 10) {
  let failures = 0;
  const findings = [];
...
function renderGithub(result, lang = "en") {
  const lines = [];
...
function annotation(finding, lang = "en") {
  const level = COMMAND_LEVEL[finding.severity];
  const rule = ruleById(finding.rule);
  const title = lang === "en" ? rule?.titleEn ?? finding.rule : rule?.title ?? "";
  const body = lang === "en" ? finding.messageEn : finding.message;
  const properties = [
    finding.source ? `file=${escapeProperty(workspaceRelative(finding.source.file))}` : void 0,
    finding.source ? `line=${finding.source.line}` : void 0,
    `title=${escapeProperty(`${finding.rule} ${title}`.trim())}`
  ].filter(Boolean).join(",");
  const text = [
    body,
    finding.suggestedDDL.length ? `DDL: ${finding.suggestedDDL.join(" ")}` : void 0,
    finding.rewrite ? `rewrite: ${finding.rewrite}` : void 0
  ].filter(Boolean).join(" | ");
...
function renderTerminal(result, options = {}) {
  ...
  if (result.findings.length === 0) {
    ...
    lines.push(
      [
        ...
        pc.dim(`\xB7 ${result.records} ${t.fingerprints}`)
      ].join("  ")
    );
  }
  ...
  if (result.findings.length > top) {
    lines.push(
      pc.dim(
        lang === "en" ? `${t.more}${result.findings.length - top}${t.moreTail}` : `${t.more}${result.findings.length - top}${t.moreTail}`
      )
    );
  }
...
function renderFinding(finding, ordinal, lang) {
  const t = LABELS[lang];
  const style = SEVERITY_STYLE[finding.severity];
  const where = [
    finding.table,
    finding.source ? `${workspaceRelative(finding.source.file)}:${finding.source.line}` : void 0,
    finding.queryTime !== void 0 ? `${finding.queryTime.toFixed(3)}s` : void 0,
    finding.rowsExamined !== void 0 ? `${finding.rowsExamined.toLocaleString("en-US")} rows` : void 0,
    (finding.occurrences ?? 1) > 1 ? `\xD7${finding.occurrences}` : void 0
...
function registerTitles(pairs) {
  titles = new Map(
    pairs.map((p) => [p.id, { title: p.title, titleEn: p.titleEn ?? p.title }])
  );
}
function findingTitle(finding, lang) {
...
function wrap(text, indent, label) {
  ...
  return lines.map(
    (line, index) => index === 0 && label ? `   ${label} ${line.padEnd(0)}` : `${pad}${line}`
  );
}
function renderFooter(result, lang) {
  ...
  if (result.skipped.length > 0) {
    ...
    for (const [reason, ids] of groups) {
      out.push(`${pc.yellow(t.skipped)} ${ids.join(", ")} \xB7 ${reason}`);
    }
  }
  for (const error of result.errors) {
...
function exitCodeFor(result, failOn) {
  ...
  return result.findings.some((f) => SEVERITY_ORDER[f.severity] >= threshold) ? EXIT_FINDINGS : EXIT_OK;
}
...
function createOpenAiCompatProvider(config) {
  return {
    ...
    async complete(prompt) {
      ...
      const timeout = setTimeout(() => controller.abort(), config.timeoutMs ?? 15e3);
      try {
        const response = await fetch(`${config.baseUrl}/chat/completions`, {
          ...
          body: JSON.stringify({
            ...
            messages: [
              {
                role: "system",
                content: "\u4F60\u4E3A MySQL \u7D22\u5F15\u4F53\u68C0\u5DE5\u5177\u8865\u5145\u8FD0\u7EF4\u63D0\u793A\u3002\u53EA\u8F93\u51FA\u4E2D\u6587\u6B63\u6587\uFF0C\u4E0D\u8981\u590D\u8FF0\u7ED3\u8BBA\uFF0C\u4E0D\u8981\u6DFB\u52A0\u514D\u8D23\u58F0\u660E\u6216\u6807\u9898\u3002"
              },
        ...
        throw new Error(
          err.name === "AbortError" ? `LLM \u8BF7\u6C42\u8D85\u65F6\uFF08${config.timeoutMs ?? 15e3}ms\uFF09` : err.message
        );
      } finally {
...
var EXIT_HELP = "\n\u9000\u51FA\u7801\uFF1A0 \u65E0\u963B\u585E\u95EE\u9898 \xB7 1 \u8FBE\u5230 --fail-on \u7684\u53D1\u73B0 \xB7 2 \u8FD0\u884C\u9519\u8BEF";
function fail(message) {
  process.stderr.write(`${pc2.red("\u9519\u8BEF")} ${message}
`);
  process.exit(EXIT_ERROR);
}
function severity(value, flag) {
...
function addAnalysisFlags(cmd) {
  cmd.enablePositionalOptions();
  return cmd.option("--format <fmt>", "table | json | github", "table").option("--schema <path>", "schema.json; unlocks index-aware rules").option("--llm", "polish the explanation text via an OpenAI-compatible endpoint").option("--emit-sql <path>", "write deduplicated DDL to a migration file").option("--top <n>", "maximum findings to print", "50").option("--min-severity <s>", "drop quieter findings", "info").option("--fail-on <s>", "exit 1 only at or above this severity", "error").option("--mysql-version <v>", "5.7 | 8.0", "8.0").option("--prefix-bytes <n>", "index key budget before recommending a prefix", "3072").option("--deep-offset <n>", "LIMIT offset considered deep", "10000").option("--rules <ids>", "comma-separated allow-list, e.g. SIA001,SIA004").option("--lang <lang>", "auto | zh | en (terminal and GitHub annotations)", "auto");
}
async function emit(source, flags, label, load = {}) {
  if (!load.inline && !existsSync3(source)) fail(`\u627E\u4E0D\u5230\u8F93\u5165\uFF1A${source}`);
  let schema;
  if (flags.schema) {
    const loaded = loadSchema(flags.schema);
    if (loaded.errors.length > 0) {
      for (const err of loaded.errors) process.stderr.write(`${pc2.red("schema \u6821\u9A8C\u5931\u8D25")} ${err}
`);
      process.exit(EXIT_ERROR);
    }
  ...
  if (flags.llm) {
    ...
    if (!config) {
      process.stderr.write(`${pc2.yellow("\u63D0\u793A")} \u672A\u8BBE\u7F6E SIA_LLM_BASE_URL\uFF0C--llm \u56DE\u9000\u5230\u79BB\u7EBF mock\u3002
`);
      llm = mockProvider;
    } else {
  ...
  const enabledRules = flags.rules ? ALL_RULES.filter(
    (rule) => flags.rules.split(",").map((part) => part.trim().toLowerCase()).filter(Boolean).includes(rule.id.toLowerCase())
  ) : void 0;
  ...
  const lang = resolveLang(flags.lang ?? "auto");
  switch (flags.format) {
    case "json":
...
`);
  ...
  if (flags.emitSql) {
    const target = resolve2(flags.emitSql);
    writeFileSync(target, renderMigration(result, label, { lang }), "utf8");
    process.stderr.write(`${pc2.green("\u2713")} \u8FC1\u79FB\u6587\u4EF6\u5DF2\u5199\u5165 ${target}
`);
  }
...
addAnalysisFlags(
  program.name("sia").description("sql-index-advisor \u2014 offline index advisor for MySQL / MyBatis").version(VERSION).argument("[input]", "slow query log / .sql file / mapper XML file or directory").addHelpText(
    "after",
    `
\u793A\u4F8B\uFF1A
...
addAnalysisFlags(
  program.command("query").description("analyse inline SQL (one statement, or several separated by ;)").argument("<sql...>", "SQL text").addHelpText("after", EXIT_HELP)
).action(async (sqlParts, flags) => {
  await emit(sqlParts.join(" "), flags, "inline-sql", { inline: true });
});
addAnalysisFlags(
  program.command("mapper").description("scan MyBatis mapper XML files or a directory").argument("<path>", "a .xml file or a directory").addHelpText("after", EXIT_HELP)
).action(async (path, flags) => {
  await emit(path, flags, path, { kind: "mapper" });
});
program.command("list").description("show what the parsers found, without running any rule").argument("<input>", "slow query log / .sql / mapper path").option("--format <fmt>", "table | json", "table").action((input, flags) => {
  if (!existsSync3(input)) emitListInline(input, flags);
  else printList(loadInput(input), flags, input);
});
function emitListInline(sql, flags) {
  printList(loadInput(sql, { inline: true }), flags, "inline-sql");
}
function printList(loaded, flags, label) {
...
`);
  ...
  for (const row of rows) {
    process.stdout.write(
      `${pc2.dim(String(row.line ?? "-").padStart(4))}  ${row.kind.padEnd(6)} ${(row.tables.join(",") || "-").padEnd(18)} ${pc2.dim(`p=${row.predicates}`)} ${row.fingerprint.slice(0, 66)}
`
    );
    for (const note of row.notes) process.stdout.write(`       ${pc2.yellow("!")} ${note}
`);
  }
  process.stdout.write(`
${pc2.bold(String(rows.length))} \u4E2A\u6307\u7EB9
`);
  for (const note of loaded.notes) process.stdout.write(`${pc2.yellow("!")} ${note}
`);
}
program.command("mcp").description("start the MCP server on stdio (for Claude / Qoder / Cursor)").action(async () => {
  const { main: main2 } = await Promise.resolve().then(() => (init_mcp(), mcp_exports));
  await main2();
  await new Promise((resolvePromise) => {
    process.stdin.once("end", () => resolvePromise());
  });
});
program.command("rules").description("print the rule catalogue as JSON").action(() => {
  process.stdout.write(`${JSON.stringify(ruleCatalogue(), null, 2)}
`);
});
program.parseAsync(process.argv).catch((err) => {
  process.stderr.write(`${pc2.red("\u8FD0\u884C\u9519\u8BEF")} ${err.message ?? String(err)}
`);
  process.exit(EXIT_ERROR);
});
